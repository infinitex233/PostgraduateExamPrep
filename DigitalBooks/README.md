...
- `scripts/query.py` 会自动读取 `DigitalBooks/Cache/*.docling.json`，并兼容逐页 OCR 缓存和旧 Docling 结构缓存。
- 408 四本书与数学三本书当前都已采用逐页 OCR 缓存，可作为查找候选 PDF 页的第一入口。
- 缓存中的 `page_no` 是 PDF 页码，不一定等于书内印刷页码。回答知识点所在页时，必须同时给出并清楚标注 `书内印刷页码` 和 `PDF 页码`；若某一项暂时不能确认，必须明确说明“书内印刷页码未确认”或“PDF 页码未确认”，并优先核对 PDF 页面补齐。
- 若缓存命中内容足以定位知识点，可先用缓存结果确定候选页；若需要精确原文、公式、图表、例题细节、书内页码，必须再打开或渲染对应 PDF 页核对。
- 若缓存没有命中，不代表教材一定没有该内容。应尝试同义关键词、缩短关键词、分词查询；仍无法确认时，再直接检查相关 PDF 页或说明“未在当前缓存/教材中确认”。
- 不要把 OCR 结果当成绝对无误的原文。OCR 文本可用于定位和初筛，最终引用应以 PDF 页面核对为准。

## Agent 查书规则

当用户询问“某个知识点书上怎么说”“在哪一页”“教材如何定义/解释”时：

- 必须先使用 `scripts/query.py` 查询本地 OCR 缓存，再按需要查阅相关 PDF 页面后回答。
- 回答应标明来源：书名、章节或小节、页码。
- 回答知识点页码时，必须同时回答并标注 `书内印刷页码` 和 `PDF 页码`。不要只给一个笼统页码。
