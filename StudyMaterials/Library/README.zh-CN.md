...
《27张宇基础30讲线代》《27张宇基础30讲概率》与王道 408 四本的缓存尚未修复，正文文字基本可用，但插图区域与公式仍可能有 OCR 错误。

## 已核验的 408 背诵手册缓存

`27计算机网络背诵手册(公众号：里昂408考研）.docling.json` 已按视觉转写流程从扫描版源 PDF（`408/27计算机网络背诵手册(公众号：里昂408考研）.pdf`，125 页 192 ppi 整页位图，无文本层）重建，并于 2026-09-22 通过校验，覆盖 125 / 125 页。

书上的标题、表格、公式、脚注、统考真题框与图注均按原样转写；图的内部以「图中文字（按图序）」列出图内可见标签，再附一段只陈述图中可见信息的说明。水印、二维码与页脚页码不转写；跨页表格的续表会标注为续上页表头。页码关系为 `书内印刷页码 + 5 = PDF 页码`（印刷 20 = PDF 25）；PDF 第 2–5 页为目录、第 125 页为「艾宾浩斯遗忘曲线」附录，两者均无印刷页码。

波形图与组帧图（图 2.3、图 2.4、图 3.3、图 3.12）图内逐位比特序列小于源位图可分辨极限，未逐位转写，这些图块只列出可辨认的标签。
