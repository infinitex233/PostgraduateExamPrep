...
class DashboardVariantTests(unittest.TestCase):
    def test_variant_renderer_outputs_single_capsule_dashboard(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("2026-03 至 2026-06", capsule_dashboard)
            self.assertIn("465h49m", capsule_dashboard)
            self.assertIn("考研相关 387h15m", capsule_dashboard)
            self.assertIn("课内/其他 78h34m", capsule_dashboard)
            self.assertIn("89天", capsule_dashboard)
            self.assertIn("距初试首日", capsule_dashboard)
            self.assertIn("172天", capsule_dashboard)
            self.assertNotIn("89 天", capsule_dashboard)
            self.assertNotIn("172 天", capsule_dashboard)
